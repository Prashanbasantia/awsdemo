...
def staff_take_attendance(request):
    ...
    # return render(request,"staff_template/staff_take_attendance.html",{"subjects":subjects,"session_years":session_years,"course":course,"students":students})
    subjects=Subjects.objects.filter(staff_id=request.user.id)
    session_years=SessionYearModel.object.all()
    course = Courses.objects.all()
    return render(request,"staff_template/staff_take_attendance.html",{"subjects":subjects,"session_years":session_years,"course":course})
...
def get_students(request):
    ...
    for student in students:
        data_small={"id":student.admin.id,"roll_no":student.admin.username,"name":student.admin.first_name+" "+student.admin.last_name}
...
def save_attendance_data(request):
    ...
    try:
        attend_exist = Attendance.objects.filter(session_year_id_id=session_year_id , attendance_date = attendance_date).exists()
        if attend_exist:
            return HttpResponse("ERREX")
        else:
...
def get_attendance_dates(request):
    ...
    attendance=Attendance.objects.filter(subject_id=subject_obj,session_year_id=session_year_obj).order_by("-id")
    attendance_obj=[]
